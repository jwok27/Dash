...
st.caption("Institutional operating system used by top macro desks • Live FRED • Updated daily")
...
# ================== FETCH DATA (institutional series) ==================
...
# Z-scores & Percentiles for Heatmap
...
def calculate_phase_score():
    ...
    if 50 < ism < 55: score += 7
    if rec_prob < 15 and credit < 2.0: score += 5
...
# ================== MACRO INTELLIGENCE BRIEFING (narrative) ==================
def generate_briefing():
    ...
    breakeven = df['5Y Breakeven Inflation'][-1]
    if score >= 80:
        return f"Contraction regime confirmed. Recession probability {rec:.1f}%. Tight financial conditions (NFCI {nfc:.2f}) and rising claims signal defensive posture. Overweight duration, gold, and high-quality credit."
    elif score >= 65:
        return f"Late-cycle expansion. Labor resilient but {claims_trend} claims + neutral financial conditions point to soft-landing bias. Inflation expectations anchored at {breakeven:.2f}%. Favor defensives, commodities, and moderate duration."
    elif score >= 40:
        return f"Mid-cycle expansion. Growth above trend, financial conditions supportive (NFCI {nfc:.2f}). ISM PMI expansionary. Risk-on bias with selective cyclicals."
    else:
        return f"Early-cycle recovery. Yield curve steep, NFCI easing. Accelerating growth momentum. Overweight equities, credit, and commodities."
...
with tab1:
    ...
    with g3: st.plotly_chart(go.Figure(go.Indicator(mode="gauge+number", value=df['ISM Mfg PMI'][-1], title={'text':"ISM PMI"}, gauge={'axis':{'range':[35,65]}})), use_container_width=True)
    with g4: st.plotly_chart(go.Figure(go.Indicator(mode="gauge+number", value=df['Initial Claims (k)'][-1], title={'text':"Wkly Claims"}, gauge={'axis':{'range':[150,400]}})), use_container_width=True)
    with g5: st.plotly_chart(go.Figure(go.Indicator(mode="gauge+number", value=df['Corp Credit Spread'][-1], title={'text':"Corp Spread"}, gauge={'axis':{'range':[1,5]}})), use_container_width=True)

with tab2:
    st.subheader("Cycle Heatmap — Historical Percentiles & Z-Scores")
    metrics = ['Unemployment Rate','10Y-3M Spread','Chicago Fed NAI','Chicago Fed NFCI','5Y Breakeven Inflation','ISM Mfg PMI','Corp Credit Spread','VIX']
    data = []
    for m in metrics:
        if m in df.columns:
            pct, z = get_percentile_and_z(df[m])
            signal = "🟢 Bullish" if z > 0.5 or (m in ['10Y-3M Spread','Chicago Fed NAI','ISM Mfg PMI'] and pct > 60) else "🔴 Caution"
            data.append([m, df[m][-1], pct, z, signal])
...
with tab3:
    ...
    fig.add_trace(go.Scatter(x=df.index, y=df['ISM Mfg PMI'], name="ISM PMI"), row=2, col=1)
...
with tab5:
    ...
    st.caption("Conviction levels derived from z-scores + cycle score. Rebalance monthly.")

st.success("✅ Billionaire-grade macro OS • All official FRED sources • Auto-updates daily")
