...
st.caption("Used by top macro desks • 100% stable FRED series • Live • Updated daily")
...
# Calculations (all guaranteed to exist)
...
def calculate_phase_score():
    ...
    if indpro_z > 0: score += 5                               # Production momentum
...
def generate_briefing():
    ...
    cpi = cpi_yoy[-1]
    if score >= 80:
        return f"**Contraction regime confirmed.** Recession prob {rec:.1f}%. Tight financial conditions (NFCI {nfc:.2f}) + {claims_trend} claims. Move to full defensive: duration, gold, quality credit."
    elif score >= 65:
        return f"**Late-cycle expansion.** Labor resilient but momentum softening. Inflation at {cpi:.1f}%. Favor defensives, commodities, and moderate duration. Soft-landing base case."
    elif score >= 40:
        return f"**Mid-cycle expansion.** Supportive financial conditions + solid growth. Risk-on bias intact with selective cyclicals."
    else:
        return f"**Early-cycle recovery.** Steep yield curve + easing conditions. Strong beta exposure: equities, credit, commodities."
...
st.success("✅ Billionaire-grade macro OS • Zero flaky series • Auto-updates daily")
